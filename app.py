...
# --- HILO SECUNDARIO PARA VERIFICAR ACTUALIZACIONES ---
class UpdateWorker(QThread):
    finished = pyqtSignal(str)

    def run(self):
        try:
            res = subprocess.run(["yt-dlp", "-U"], capture_output=True, text=True)
            output = res.stdout if res.stdout else res.stderr
            self.finished.emit(output.strip())
        except Exception as e:
            self.finished.emit(f"Error al verificar actualización: {str(e)}")

# --- HILO SECUNDARIO PARA LA DESCARGA EN TIEMPO REAL ---
class DownloadWorker(QThread):
    progress = pyqtSignal(float, str)  # Porcentaje (0-100), Texto de estado
    log = pyqtSignal(str)              # Salida de consola
    finished = pyqtSignal(bool, str)   # Éxito (True/False), Mensaje final
    ...
    def run(self):
        ...
        try:
            # Capturar salida de yt-dlp en tiempo real
            ...
            for line in process.stdout:
                ...
                if line_clean:
                    ...
                    # Extraer porcentaje si está presente en la salida de yt-dlp
...
class YtdlpApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        # 2. Selección de Ruta de Destino
        ...
        # Campo para formato personalizado (Oculto por defecto)
    ...
    def check_updates_async(self):
        self.log_text.append("[+] Verificando actualizaciones de yt-dlp...")
    ...
    def start_download(self):
        ...
        # Bloquear controles durante la descarga
